...
def convert(coordsString, decimalPlaces=5):
    # if not decimalPlaces:
    #     decimalPlaces = 5
    
    coordsString = re.sub("/\s\s+/g", " ", coordsString).strip(); #just to tidy up whitespaces
    ...
    elif re.search(dms_periods, coordsString):
        match = dms_periods.findall(coordsString)
        matchSuccess = checkMatch(match)
    ...
    elif re.search(dms_abbr, coordsString):
        match = dms_abbr.findall(coordsString)
        matchSuccess = checkMatch(match)
    ...
    elif coords_other.search(coordsString):
        match = coords_other.findall(coordsString)
        matchSuccess = checkMatch(match)
    ...
    if matchSuccess:
        ...
        ddLat = int(int(ddLat).toFixed(decimalPlaces))

        if math.isnan(ddLng) and ',' in ddLng:
            ddLng = ddLng.replace(',', '.')

        ddLng = int(int(ddLng).toFixed(decimalPlaces))
